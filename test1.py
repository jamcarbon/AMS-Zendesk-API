...
def create_ticket(event):
    try:
        ...
        *Please note: this e-mail was sent from an API that cannot accept incoming e-mail. You can send a public reply to the Zendesk ticket to send the information back to AMS.
        ...
        AMS_ticketID = event["cases"][0]["caseId"]
        ...
        zendesk_t_id = rjson["ticket"]["id"]
        caseId_str = str(event["cases"][0]["caseId"])
        zendeskId_str = str(zendesk_t_id)
        ticket_table = {
            "Id": {"S": caseId_str},
            "zendesk_t_id": {"N": zendeskId_str}
        }
        print(rjson["ticket"]["id"])
        print(type(rjson["ticket"]["id"]))
        
        register_ticketid(ticket_table)
